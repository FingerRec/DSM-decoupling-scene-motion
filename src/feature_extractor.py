...
def main():
    ...
    train_transforms, test_transforms, _ = pt_augmentation_config(args)
    train_data_loader, val_data_loader, _, _, _, _ = pt_data_loader_init(args, data_length, image_tmpl, train_transforms,
                                                                      test_transforms, _)
    # == model config==
    model = pt_model_config(args, num_class)
    tc = TC(args)
